...
- **Proxy**: proxy.py — translates Anthropic Messages API ↔ OpenAI Chat Completions API
- **Launcher**: run_claude.sh — starts proxy, launches claude CLI
...
### Token Refresh (implemented in proxy.py)
...
### Token Selection Order in proxy.py
...
3. After that, `run_claude` picks up fresh tokens automatically
